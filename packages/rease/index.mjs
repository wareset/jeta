...
import { storeDestroy as storeDestroy$1, storeSubscribe as storeSubscribe$1, storeOnSubscribe as storeOnSubscribe$1, storeOnDestroy as storeOnDestroy$1, storeOnUpdate as storeOnUpdate$1, storeOnChange as storeOnChange$1, isStore, store as store$1 } from '@wareset/store';
export { Store, isStore, store as storeModule, storeDestroy as storeModuleDestroy, storeOnChange as storeModuleOnChange, storeOnDestroy as storeModuleOnDestroy, storeOnSubscribe as storeModuleOnSubscribe, storeOnUpdate as storeModuleOnUpdate, storeSubscribe as storeModuleSubscribe } from '@wareset/store';
...
import { MAX_SAFE_INTEGER } from '@wareset-utilites/number/MAX_SAFE_INTEGER'; // prettier-ignore

var storeReadonlyfy = store => (defineProperty(store, 'readonly', {
  value: true
...
var storeModuleProxy = storeProxyFactory(store$1);

var storeFactory = (type, fn) => rease => isNill(rease) ? fn : __(rease)[type]; // prettier-ignore


var store = storeFactory("EH_SERVICE_store"
/* store */
, store$1); // prettier-ignore
...
var getStoreValue = value => isStore(value) ? value.get() : value; // prettier-ignore


var storefy = rease => (value, instance) => instance || !isStore(value) ? store(rease)(value) : value;
...
var __normalizePropertyOptions__ = options => !options ? {} : isArray(options = isFunction(options) ? [options] : options) ? {
  [TYPED]: options
...
(_rease, _props, _options) => {
  ...
  if (_props) {
    var propKeys = keys(isObject(_props = getStoreValue(_props)) ? _props : [_props]);
    var isObjRes = isObject(_props) && !isArray(_props);
    res = isObjRes ? {} : []; // prettier-ignore

    var optionsGlobal = __normalizePropertyOptions__(_options);

    forEachLeft(propKeys, _k => {
      var prop = _props[_k];
      var isObj = isObject(prop);
      var name = isObj ? prop.name || _k : prop;
      var options = isString(prop) ? optionsGlobal : __normalizePropertyOptions__(prop);
      var isset;
      var property = (isset = hasOwnProperty(properties, name)) ? properties[name] : options.default;
      ...
      if (hasOwnProperty(options, VALUE) && !includes(tmp = __arrayfy__(options[VALUE]), propertyValue)) {
        throwError(name + ' is not ' + VALUE + ': ' + tmp);
      }

      if (hasOwnProperty(options, STORE) && !options[STORE] === (tmp = isStore(property))) {
        throwError(name + ' is ' + STORE + ' must by ' + !tmp);
      }

      if (hasOwnProperty(options, TYPED) && !typed(propertyValue, ...(tmp = __arrayfy__(options[TYPED]))) || hasOwnProperty(options, TYPEDOF) && !typedOf(propertyValue, ...(tmp = __arrayfy__(options[TYPEDOF])))) {
        throwError(name + ' is not ' + TYPED + ': ' + tmp.map(v => v.name));
      } // prettier-ignore


      var propertyFinal = hasOwnProperty(options, STOREFY) ? options[STOREFY] ? storefy(_rease)(property) : propertyValue : property; // console.log(111, name, propertyValue, options)
...
var isArrayNotStore = value => isArray(value) && !isStore(value); // https://developer.mozilla.org/en-US/docs/Web/API/WindowOrWorkerGlobalScope/queueMicrotask
// prettier-ignore
...
var createStoresService = () => {
  ...
  var store = (...a) => (stores.unshift(a = store$1(...a)), a);

  var storeProxy = storeProxyFactory(store); // prettier-ignore

  var storeDestroy = (...a) => {
    forEachRight(isStore(a[0]) ? [a[0]] : a[0], v => {
      spliceWith(stores, v, 1);
    }), storeDestroy$1(...a);
  };

  var storeSubscribe = __storeOn__(unsubscribers, storeSubscribe$1);

  var destroy = () => {
    forEachRight(unsubscribers, operate), clear(unsubscribers);
    forEachRight(unlisteners, operate), clear(unlisteners);
    storeDestroy$1(stores), clear(stores);
  };
...
var createServiceRease = _self => {
  ...
  var service = {
    ...
    /* storeOnSubscribe */
    ]: __storeOn__(unlisteners, storeOnSubscribe$1),
    ["EH_SERVICE_storeOnDestroy"
    /* storeOnDestroy */
    ]: __storeOn__(unlisteners, storeOnDestroy$1),
    ["EH_SERVICE_storeOnUpdate"
    /* storeOnUpdate */
    ]: __storeOn__(unlisteners, storeOnUpdate$1),
    ["EH_SERVICE_storeOnChange"
    /* storeOnChange */
    ]: __storeOn__(unlisteners, storeOnChange$1),
...
var isRease = v => instanceOf(v, __Rease__);
...
class __Props__ {
  constructor(_props) {
    ...
    if (_props = getStoreValue(_props)) {
      ...
      if (isArray(_props)) {
        forEachLeft(_props, // isArray(propsDirty[0]) ? propsDirty : [propsDirty],
        _data => {
          var [k, ...v] = _data;
...
var createProps = _props => instanceOf(_props, __Props__) ? _props : new __Props__(_props); // export const getChildsTokensList = (
...
var rnodeUtilsGetSystemProperty = (_store, _props, _name, _default) => {
  var rules = concat(...(_props["EH_PROPS_system"
  /* system */
  ][_name] || []));

  var $rule = _store(last(rules), rules);

  return isUndefined(_default) ? $rule : _store($rule, a => isUndefined(a = $rule.get()) ? _default : a);
};
...
var isChildlessTagName = tagName => hasOwnProperty(CHILDLESS_TAGNAMES, tagName);
...
class ReaseSlot extends __Rease__ {
  __(_serviceRease, _thisRNode, _props) {
    ...
    var _children = {};
    var $children = store();
    forEachLeft(outsideNodes, (_node, _key) => {
      // prettier-ignore
...
class ReaseText extends __Rease__ {
  __(_serviceRease, _thisRNode) {
    ...
    var $wholeText = storeProxy(childs, () => childs.map(v => isNill(v = getStoreValue(v)) ? '' : isObject(v) ? trycatch(() => jsonStringify(v, undef, 2), () => v) : v).join('')); // prettier-ignore
...
var __getSub__ = (_thisRNode, storeProxy, subs, subsOld, children, _key, _v, _k, _a) => {
  ...
  var sub = findLeft(subsOld, v => v[0] === key);
...
var __ruleElse__ = (_thisRNode, storeSubscribe, _$ruleElse) => {
  ...
  if (reaseClassesPrev[0]) {
    ...
    storeSubscribe(reaseClassesPrev, (...a) => {
      ...
      someRight(a, (_class, _k) => ((_class === RuleIf || _class === RuleElseIf) && (isRNodeId = _k), _class === RuleElse || isRNodeId > -1));
      var thisRNodeIf;
...
var __destroyRease__ = (_thisRease, _full) => {
  ...
  /* destroyed */
    ...
    forEachLeft(_serviceRease["EH_SERVICE_beforeDestroyList"
    /* beforeDestroyList */
    ], operate); // prettier-ignore

    forEachRight(_serviceRease["EH_SERVICE_readonly"
    /* readonly */
    ]["EH_SERVICE_children"
    /* children */
    ], rease => {
    ...
    forEachLeft(_serviceRease["EH_SERVICE_onDestroyList"
    /* onDestroyList */
    ], operate);
    ...
    clear(_serviceRease["EH_SERVICE_beforeCreateList"
    /* beforeCreateList */
    ]);
    clear(_serviceRease["EH_SERVICE_onCreateList"
    /* onCreateList */
    ]);
    clear(_serviceRease["EH_SERVICE_beforeMountList"
    /* beforeMountList */
    ]);
    clear(_serviceRease["EH_SERVICE_onMountList"
    /* onMountList */
    ]);
    clear(_serviceRease["EH_SERVICE_beforeDestroyList"
    /* beforeDestroyList */
    ]);
    clear(_serviceRease["EH_SERVICE_onDestroyList"
    /* onDestroyList */
...
var __destroyRNode__ = _thisRNode => {
  ...
  /* destroyed */
    ...
    forEachRight(_thisRNode["EH_RNODE_$children"
    /* $children */
    ].get(), v => {
      __destroyRNode__(v);
    });
...
class __RNode__ {
  constructor(_tagOrRease, _props, _childs, _homeRNode, _parentRNode) {
    ...
    /* childIdExecute */
    ] = () => (++childId < MAX_SAFE_INTEGER || (childIdTemp += childId, childId = 1), thisRNode["EH_RNODE_id"
    /* id */
    ] + '-' + childIdTemp + childId);
    ...
    storeProxyRNode([_tagOrRease], () => {
      ...
      /* destroyed */
        ...
        var reaseClass = isString(tagOrRease) ? (tagName = this._tagName = tagOrRease, hasOwnProperty(reases, tagOrRease) ? reases[tagOrRease] : ReaseElement) : isRease(tagOrRease) ? getPrototypeOf(tagOrRease) : tagOrRease;
        ...
        if (reaseClassOld === (reaseClassOld = reaseClass)) {
          ...
          forEachLeft([[_homeRNode, "EH_SERVICE_$home"
          /* $home */
          , "EH_SERVICE_home"
          /* home */
...
export { RuleAwait as RAwait, RuleCatch as RCatch, ReaseComment as RComment, ReaseComponent as RComponent, ReaseElement as RElement, RuleElse as RElse, RuleElseIf as RElseIf, RuleFor as RFor, RuleForIn as RForIn, RuleForOf as RForOf, ReaseFragment as RFragment, RuleIf as RIf, ReaseMixin as RMixin, ReaseRoot as RRoot, ReaseRule as RRule, ReaseScript as RScript, ReaseSlot as RSlot, ReaseStyle as RStyle, ReaseText as RText, RuleThen as RThen, beforeCreate, beforeDestroy, beforeMount, createReaseDOM, createReaseSSR, createTree, getProperty, isArrayNotStore, onCreate, onDestroy, onMount, resolve, store, storeDestroy, storeModuleProxy, storeOnChange, storeOnDestroy, storeOnSubscribe, storeOnUpdate, storeProxy, storeSubscribe };
