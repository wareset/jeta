...
const CDATA_AND_COMMENT = [
  // ['cdata', '<![CDATA[', ']]>'],
  [TAGNAME_CDATA, TAG_OPENER_CDATA, TAG_CLOSER_CDATA],
  // ['comment', '<!--', '-->']
...
export default function createNodelist(html) {
  ...
  data: for (let i = 0; i < data.length; i++) {
    ...
    if (first(tag, SIGN_LESS) && tag[1].match(/[/!{\w]/)) {
      ...
      if (tag[1] === SIGN_SLASH) (node[1] = CLOSER), deep--;
      else if (tag[1] === '!' || last(tag, SIGN_SLASH_MORE)) node[1] = SAMELESS;
      else (node[1] = OPENER), deep++, (node[0] = deep);
    } else {
      index = resultDirty[resultDirty.length - 1];
      if (index && index[1] === TAGNAME_TEXT) index[3] += data[i];
      else resultDirty.push((node = [deep, TAGNAME_TEXT, '', data[i], '']));
    }
  }
