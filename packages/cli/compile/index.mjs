...
import { getAlias, toKebabCase, getCID, getCIDArr, lower } from './lib';
...
const prebuildStyle = (node, result) => {
    ...
    const css = isConcise
        ? (v) => rastree.css.concise(v)
        : (v) => rastree.css(parseLang(v));
    const styles = [];
...
const normalizeAttr = (raw, result, minify) => {
    const resArr = typeof raw === 'string' ? parseHTMLX(raw, false) : [{ deep: 0, raw }];
...
const prebuildTemplate = (node, result) => {
    ...
    const html = isConcise
        ? (v) => rastree.html.concise(v)
        : (v) => rastree.html(parseLang(v));
    const templates = [];
    node.children.forEach((v) => {
        if (typeof v.data === 'string')
...
const createFinal = (result) => {
    const alias = (v) => (result.temp['__' + v] = getAlias(JSON.stringify(result), v, false));
    ...
    Object.keys(result.temp.attributes).forEach((origin, key) => {
        cache.push('\n\t' + origin);
        // prettier-ignore
        template = template.replace(regexp(escape(result.temp.attributes[origin]), 'g'), `${__cache__}[${key}]`);
    });
    let key = -1;
    const helpers = [];
    ['tag', 'store', 'text', 'cdata', 'comment', 'spread'].forEach((v) => {
        ...
        // prettier-ignore
        template = template.replace(regexp(escape(result.temp[v]), 'g'), `${__helpers__}[${++key}]`);
    });
...
function compile(content = '', optionsStart = OPTIONS) {
    ...
    options.cid = getCID(options.cidSalt || options.name || '', options.cidType, options.cid);
    const res = {
        content,
        options,
        temp: { attributes: {} },
